...
const Sidebar = (props) => {
  ...
  const [currentUser, ...chatUsersList] = getChatListWithOnlineUser() || [];

  return (
    <div className="col-12 col-sm-4 col-md-4 col-lg-4 col-xl-2 border-right">
      <div className="d-none d-md-block" style={{ paddingTop: "20px" }}>
        <Link
          onClick={() => props.setCurrentChattingMember(currentUser)}
          to={`/c/${currentUser?.roomId}`}
          className={
            "pl-1 list-group-item list-group-item-action border-0 " +
            getActiveChatClass(currentUser?.roomId)
          }
          key={currentUser?.id}
        >
