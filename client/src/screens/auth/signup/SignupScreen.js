...
const SignupScreen = ({ history }) => {
  ...
  const onSubmit = async (signupData) => {
    ...
    if (successSignupData) {
      ...
      handleSignupError(successSignupData);
    }
  };
  const handleSignupError = (response) => {
    if (response && response.detail) {
      setSignupError(response.detail); // Set the error message from the response
    } else {
      setSignupError("choose strong password."); // Fallback message
    }
  };
