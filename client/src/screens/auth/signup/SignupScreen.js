  ...
  const onSubmit = async (signupData) => {
    ...
    Object.keys(signupData).forEach((key) => {
      if(key === "password" || key === "passwordTwo"){
        formData.append(key, CryptoJS.SHA256(signupData[key]).toString());
      }
      else{
    ...
    // console.log("successSignupData", successSignupData);
    if (successSignupData) {
      ...
      handleSignupError(successSignupData.data);
    }
  };

  const handleSignupError = (response) => {
    if (response) {
      setSignupError(response);
    } else {
      setSignupError("account already exist with this email"); 
    }
  };
    ...
    <div id="authFormContainer">
      <div id="authForm">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <br />
          {signupError && <p className="requiredFieldError">{signupError}</p>}
          <br />
