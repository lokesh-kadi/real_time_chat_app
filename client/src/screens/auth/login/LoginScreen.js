...
const LoginScreen = ({ location }) => {
  ...
  const onSubmit = async (loginData) => {
    ...
    console.log(successLoginData.data)
    if (successLoginData) {
  ...
  const handleLoginError = (response) => {
    if (response && response.detail) {
      setLoginError(response.detail); // Set the error message from the response
    } else {
      setLoginError("Invalid email or password."); // Fallback message
    }
  };
    ...
    <div id="authFormContainer">
      <div id="authForm">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <br />
          {loginError && <p className="requiredFieldError">{loginError}</p>}
          <br />
